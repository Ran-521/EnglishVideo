...
import numpy as np
from moviepy.editor import VideoFileClip, ImageClip, CompositeVideoClip, ColorClip, clips_array, vfx
# 导入moviepy的vfx模块
import tempfile
import cv2
...
class VideoProcessor(QThread):
    ...
    def __init__(self, video_files, offset, 
                 ...
                 bg_img_path=None, bg_img_x=0, bg_img_y=0, bg_img_scale=1.0, bg_img_opacity=0.8,
        ...
        Args:
            ...
            bg_img_path: 背景叠加图片路径
            bg_img_x: 背景图片X坐标
            bg_img_y: 背景图片Y坐标
            bg_img_scale: 背景图片缩放比例
            bg_img_opacity: 背景图片透明度
        ...
        self.bg_img_x = bg_img_x
        self.bg_img_y = bg_img_y
        self.bg_img_scale = bg_img_scale
        self.bg_img_opacity = bg_img_opacity
    ...
    def process_preview(self):
        ...
        try:
            ...
            # 使用正片叠底方式添加背景图片
            final_frame = moved_frame.copy()
            if self.bg_img_path and os.path.exists(self.bg_img_path):
                try:
                    # 加载背景图片作为静态图像
                    bg_img_pil = Image.open(self.bg_img_path)
                    # 调整大小
                    new_width = int(video_width * self.bg_img_scale)
                    new_height = int(bg_img_pil.height * new_width / bg_img_pil.width)
                    bg_img_pil = bg_img_pil.resize((new_width, new_height))
                    # 转换为numpy数组
                    bg_array = np.array(bg_img_pil)
                    
                    # 确保背景图片有正确的通道数
                    if len(bg_array.shape) == 2:  # 灰度图像
                        bg_array = np.stack([bg_array, bg_array, bg_array], axis=2)
                    elif bg_array.shape[2] == 4:  # 带透明通道
                        # 将透明通道与RGB通道分离
                        alpha = bg_array[:, :, 3] / 255.0
                        bg_array = bg_array[:, :, :3]
                    
                    # 计算放置位置
                    x_pos = int(self.bg_img_x)
                    y_pos = int(self.bg_img_y)
                    
                    # 确保背景图片不会超出边界
                    frame_h, frame_w = moved_frame.shape[:2]
                    bg_h, bg_w = bg_array.shape[:2]
                    
                    # 裁剪背景图片确保不超出边界
                    if x_pos < 0:
                        bg_array = bg_array[:, -x_pos:] if -x_pos < bg_w else None
                        bg_w = bg_array.shape[1] if bg_array is not None else 0
                        x_pos = 0
                    
                    if y_pos < 0:
                        bg_array = bg_array[-y_pos:, :] if -y_pos < bg_h else None
                        bg_h = bg_array.shape[0] if bg_array is not None else 0
                        y_pos = 0
                    
                    if x_pos + bg_w > frame_w:
                        bg_array = bg_array[:, :frame_w-x_pos] if bg_array is not None else None
                        bg_w = bg_array.shape[1] if bg_array is not None else 0
                    
                    if y_pos + bg_h > frame_h:
                        bg_array = bg_array[:frame_h-y_pos, :] if bg_array is not None else None
                        bg_h = bg_array.shape[0] if bg_array is not None else 0
                    
                    # 应用正片叠底效果
                    if bg_array is not None and bg_w > 0 and bg_h > 0:
                        # 1. 获取视频帧对应区域
                        frame_region = moved_frame[y_pos:y_pos+bg_h, x_pos:x_pos+bg_w].astype(np.float32)
                        # 2. 将背景图片转换为float进行乘法操作
                        bg_float = bg_array.astype(np.float32)
                        # 3. 正片叠底计算
                        multiply_result = (frame_region * bg_float) / 255.0
                        # 4. 混合原始帧和正片叠底结果
                        final_region = frame_region * (1 - self.bg_img_opacity) + multiply_result * self.bg_img_opacity
                        # 5. 将结果裁剪到0-255范围并转回uint8
                        final_region = np.clip(final_region, 0, 255).astype(np.uint8)
                        # 6. 将处理后的区域放回最终帧
                        final_frame[y_pos:y_pos+bg_h, x_pos:x_pos+bg_w] = final_region
                    
                        self.error_occurred.emit(f"已添加背景图片(正片叠底模式): {os.path.basename(self.bg_img_path)}")
                except Exception as e:
                    self.error_occurred.emit(f"添加背景图片失败: {str(e)}")
                    traceback.print_exc()
            
            # 显示预览帧
            self.preview_frame_ready.emit(final_frame)
        ...
        try:
            ...
            for idx, video_path in enumerate(self.video_files):
                ...
                try:
                    ...
                    # 使用正片叠底方式添加背景图片
                    final_clip = final_clip.fl(lambda gf, t: self.process_background(gf(t)))
    ...
    def process_background(self, frame):
        """处理背景"""
        # 使用正片叠底方式添加背景图片
        if self.bg_img_path and os.path.exists(self.bg_img_path):
            try:
                ...
                bg_img_pil = Image.open(self.bg_img_path)
                # 调整大小
                new_width = int(frame.shape[1] * self.bg_img_scale)
                new_height = int(bg_img_pil.height * new_width / bg_img_pil.width)
                bg_img_pil = bg_img_pil.resize((new_width, new_height))
                # 转换为numpy数组
                bg_array = np.array(bg_img_pil)
                ...
                if bg_array is not None and bg_w > 0 and bg_h > 0:
                    # 1. 获取视频帧对应区域
                    frame_region = frame[y_pos:y_pos+bg_h, x_pos:x_pos+bg_w].astype(np.float32)
                    # 2. 将背景图片转换为float进行乘法操作
                    bg_float = bg_array.astype(np.float32)
                    # 3. 正片叠底计算
                    multiply_result = (frame_region * bg_float) / 255.0
                    ...
                    final_region = np.clip(final_region, 0, 255).astype(np.uint8)
                    # 6. 将处理后的区域放回最终帧
                    frame[y_pos:y_pos+bg_h, x_pos:x_pos+bg_w] = final_region
...
class VideoEditorApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.bg_img_path = "E:\\Onedrive\\KMS\\10-英语音频\\赵老师做好的\\01-图片\\背景图片.png"
    ...
    def create_image_settings_group(self, parent_layout):
        ...
        # 背景图片设置
        bg_img_group = QGroupBox("背景图片设置(正片叠底)")
        bg_img_layout = QGridLayout(bg_img_group)
        
        # 背景图片路径
        bg_img_path_layout = QHBoxLayout()
        bg_img_path_layout.addWidget(QLabel("路径:"))
        self.bg_img_path_label = QLabel(os.path.basename(self.bg_img_path) if self.bg_img_path else "未选择")
        self.bg_img_path_label.setStyleSheet("font-style: italic; color: #666666;")
        bg_img_path_layout.addWidget(self.bg_img_path_label, 1)
        self.select_bg_img_btn = QPushButton("选择")
        self.select_bg_img_btn.setMaximumWidth(60)
        self.select_bg_img_btn.clicked.connect(lambda: self.select_image(3))
        bg_img_path_layout.addWidget(self.select_bg_img_btn)
        
        bg_img_layout.addLayout(bg_img_path_layout, 0, 0, 1, 2)
        
        # 背景图片缩放
        bg_img_layout.addWidget(QLabel("缩放比例:"), 1, 0)
        self.bg_img_scale_spin = QDoubleSpinBox()
        self.bg_img_scale_spin.setRange(0.1, 2.0)
        self.bg_img_scale_spin.setValue(1.0)
        self.bg_img_scale_spin.setSingleStep(0.05)
        self.bg_img_scale_spin.valueChanged.connect(self.update_preview)
        bg_img_layout.addWidget(self.bg_img_scale_spin, 1, 1)
        
        # 背景图片右移
        bg_img_layout.addWidget(QLabel("右移像素:"), 2, 0)
        self.bg_img_x_spin = QSpinBox()
        self.bg_img_x_spin.setRange(0, 1920)
        self.bg_img_x_spin.setValue(0)
        self.bg_img_x_spin.setSingleStep(10)
        self.bg_img_x_spin.valueChanged.connect(self.update_preview)
        bg_img_layout.addWidget(self.bg_img_x_spin, 2, 1)
        
        # 背景图片下移
        bg_img_layout.addWidget(QLabel("下移像素:"), 3, 0)
        self.bg_img_y_spin = QSpinBox()
        self.bg_img_y_spin.setRange(0, 1080)
        self.bg_img_y_spin.setValue(0)
        self.bg_img_y_spin.setSingleStep(10)
        self.bg_img_y_spin.valueChanged.connect(self.update_preview)
        bg_img_layout.addWidget(self.bg_img_y_spin, 3, 1)
        
        # 背景图片透明度
        bg_img_layout.addWidget(QLabel("混合强度:"), 4, 0)
        self.bg_img_opacity_spin = QDoubleSpinBox()
        self.bg_img_opacity_spin.setRange(0.1, 1.0)
        self.bg_img_opacity_spin.setValue(0.8)
        self.bg_img_opacity_spin.setSingleStep(0.05)
        self.bg_img_opacity_spin.valueChanged.connect(self.update_preview)
        bg_img_layout.addWidget(self.bg_img_opacity_spin, 4, 1)
        
        # 添加到主布局
        layout.addWidget(img1_group)
        layout.addWidget(img2_group)
        layout.addWidget(bg_img_group)
    ...
    def select_image(self, img_number):
        ...
        if file_path:
            if img_number == 1:
                ...
                self.bg_img_path = file_path
                self.bg_img_path_label.setText(os.path.basename(file_path))
    ...
    def generate_preview(self):
        ...
        if self.bg_img_path and not os.path.exists(self.bg_img_path):
            QMessageBox.warning(self, "警告", f"背景图片不存在: {self.bg_img_path}")
            return
        ...
        self.processor = VideoProcessor(
            ...
            self.bg_img_path,
            self.bg_img_x_spin.value(),
            self.bg_img_y_spin.value(),
            self.bg_img_scale_spin.value(),
            self.bg_img_opacity_spin.value(),
            preview_only=True
        )
